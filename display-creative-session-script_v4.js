...
(function(omidGlobal) {
    ...
    var PubMaticImpressionOcc = false;
    verificationClient.addEventListener('geometryChange', function(event) {
        try {
          //document.getElementById("p1").innerHTML = JSON.stringify(event.data["adView"]["onScreenContainerGeometry"]["height"]);
          //let onScreenContainerHeight = JSON.stringify(event.data["adView"]["onScreenContainerGeometry"]["height"]);
          let onScreenGeometryPixels = JSON.stringify(event.data["adView"]["onScreenGeometry"]["pixels"]);
          let percentageInView = JSON.stringify(event.data["adView"]["percentageInView"]);
          document.getElementById("p1").innerHTML = percentageInView
          ...
          if (onScreenGeometryPixels > 0 && !this.PubMaticImpressionOcc) {
            fireURL("https://pubmatic.com/om/impressiontracker");
            this.PubMaticImpressionOcc = true
          }
        }catch(err) {
          document.getElementById("p1").innerHTML += "err " + err;
        }
